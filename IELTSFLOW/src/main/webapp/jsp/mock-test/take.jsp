...
<head>
    ...
    <style>
        ...
        body { margin:0; padding:0; background:#0f172a !important; color:#f1f5f9 !important; font-family:inherit; }
        ...
        .exam-title span{color:#94a3b8;font-weight:400;margin-left:.5rem;font-size:.875rem;}
        .timer{display:flex;align-items:center;gap:.5rem;font-size:1.5rem;font-weight:800;font-variant-numeric:tabular-nums;color:#6366f1;}
        .timer.warning{color:#f59e0b;}
        .timer.danger{color:#ef4444;animation:blink .8s infinite;}
        @keyframes blink{0%,100%{opacity:1}50%{opacity:.3}}
        .violation-bar{display:flex;align-items:center;gap:.5rem;font-size:.85rem;color:#94a3b8;}
        .violation-dot{width:12px;height:12px;border-radius:50%;border:2px solid #94a3b8;}
        .violation-dot.used{background:#ef4444;border-color:#ef4444;}
        ...
        .skill-tab{
            padding:.5rem 1.25rem;border-radius:.6rem;border:1px solid rgba(255,255,255,.08);
            background:rgba(255,255,255,.05);color:#94a3b8;cursor:pointer;font-size:.875rem;
            font-family:inherit;font-weight:500;transition:all .2s;
        }
        .skill-tab.active{background:#6366f1;border-color:#6366f1;color:#fff;}
        .skill-tab:hover:not(.active){border-color:#6366f1;color:#6366f1;}

        .skill-section{display:none;}
        .skill-section.active{display:block;}

        .resource-box{
            background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:1rem;
            padding:1.5rem;margin-bottom:1.5rem;max-height:280px;overflow-y:auto;
            font-size:.9rem;line-height:1.8;color:#94a3b8;
        }
        ...
        .q-num{display:inline-block;background:rgba(99,102,241,.15);color:#6366f1;border-radius:.4rem;padding:.2rem .6rem;font-size:.75rem;font-weight:700;margin-bottom:.75rem;}
        .q-skill-badge{display:inline-block;padding:.15rem .6rem;border-radius:.4rem;font-size:.7rem;font-weight:600;text-transform:uppercase;letter-spacing:.05em;margin-left:.5rem;}
        .q-skill-badge.Listening{background:rgba(16,185,129,.15);color:#10b981;}
        .q-skill-badge.Reading{background:rgba(99,102,241,.15);color:#6366f1;}
        .q-skill-badge.Writing{background:rgba(245,158,11,.15);color:#f59e0b;}
        .q-skill-badge.Speaking{background:rgba(236,72,153,.15);color:#ec4899;}
        .q-content{font-size:1rem;line-height:1.65;color:#f1f5f9;margin-bottom:1rem;}

        .choices{display:flex;flex-direction:column;gap:.5rem;}
        .choice{
            display:flex;align-items:center;gap:.75rem;padding:.75rem 1rem;
            border:1px solid rgba(255,255,255,.08);border-radius:.6rem;cursor:pointer;
            transition:all .2s;width:100%;
        }
        .choice:hover{border-color:#6366f1;background:rgba(99,102,241,.05);}
        .choice input[type=radio]{accent-color:#6366f1;width:16px;height:16px;cursor:pointer;}
        .choice .choice-text{cursor:pointer;font-size:.9rem;line-height:1.5;width:100%;display:block;}

        .essay-area{
            width:100%;min-height:220px;padding:1rem;border-radius:.75rem;resize:vertical;
            background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);
            color:#f1f5f9;font-family:inherit;font-size:.9rem;line-height:1.7;transition:border-color .2s;
        }
        .essay-area:focus{outline:none;border-color:#6366f1;}
        .word-count{font-size:.8rem;color:#94a3b8;text-align:right;margin-top:.4rem;}

        .speaking-controls{display:flex;flex-direction:column;gap:1rem;}
        .timer-circle{
            display:flex;align-items:center;justify-content:center;
            width:80px;height:80px;border-radius:50%;
            border:3px solid #6366f1;font-size:1.2rem;font-weight:800;
            color:#6366f1;font-variant-numeric:tabular-nums;margin:0 auto;
        }
        .rec-btn{
            display:flex;align-items:center;justify-content:center;gap:.5rem;
            padding:.75rem 1.5rem;border-radius:.75rem;border:none;cursor:pointer;
            font-family:inherit;font-weight:600;font-size:.9rem;transition:all .3s;
        }
        .rec-btn.start{background:#ef4444;color:#fff;}
        .rec-btn.stop{background:rgba(255,255,255,.08);color:#f1f5f9;border:1px solid rgba(255,255,255,.08);}
        .rec-btn:hover{transform:translateY(-1px);}
        .transcript-display{
            background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:.75rem;
            padding:1rem;font-size:.85rem;color:#94a3b8;min-height:80px;line-height:1.7;font-style:italic;
        }
        ...
        .progress-info{font-size:.875rem;color:#94a3b8;}
        .nav-btns{display:flex;gap:.75rem;}
        .btn-nav{
            padding:.65rem 1.5rem;border-radius:.75rem;border:1px solid rgba(255,255,255,.08);
            background:rgba(255,255,255,.05);color:#f1f5f9;cursor:pointer;
            font-family:inherit;font-size:.9rem;font-weight:600;transition:all .2s;
        }
        .btn-nav:hover{border-color:#6366f1;}
        .btn-submit-exam{
            padding:.65rem 2rem;border-radius:.75rem;border:none;
            background:linear-gradient(135deg,#6366f1,#8b5cf6);
            color:#fff;cursor:pointer;font-family:inherit;font-size:.9rem;font-weight:700;transition:all .2s;
        }
        .btn-submit-exam:hover{transform:translateY(-1px);box-shadow:0 8px 20px rgba(99,102,241,.35);}

        /* Overlays */
        .overlay{position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:9999;display:none;flex-direction:column;align-items:center;justify-content:center;gap:1.5rem;}
        .overlay.active{display:flex;}
        .overlay-card{background:#1e293b;border:1px solid #ef4444;border-radius:1.25rem;padding:2.5rem;max-width:480px;text-align:center;}
        .overlay-card h2{color:#ef4444;font-size:1.5rem;margin-bottom:.75rem;}
        .overlay-card p{color:#94a3b8;line-height:1.7;margin-bottom:1.5rem;}
        .overlay-card.accent{border-color:#6366f1;}
        .overlay-card.accent h2{color:#6366f1;}
        .btn-back-focus{padding:.75rem 2rem;border-radius:.875rem;border:none;background:#6366f1;color:#fff;font-size:1rem;font-weight:700;cursor:pointer;font-family:inherit;}
        .forced-overlay{position:fixed;inset:0;background:rgba(0,0,0,.95);z-index:99999;display:none;flex-direction:column;align-items:center;justify-content:center;gap:1.5rem;}
        .forced-overlay.active{display:flex;}
        .forced-overlay h1{color:#ef4444;font-size:2rem;font-weight:800;}
        .forced-overlay p{color:#94a3b8;max-width:400px;text-align:center;line-height:1.7;}
...
<div class="overlay active" id="start-overlay">
    <div class="overlay-card accent" style="max-width:520px;">
        ...
        <div style="color:#94a3b8;font-size:.9rem;text-align:left;line-height:1.8;margin-bottom:1.75rem;">
            ...
            <p>Hệ thống đã kích hoạt chế độ <strong>Giám sát trực tuyến (Online Proctoring)</strong>.</p>
        ...
        <button class="btn-back-focus" id="btn-start-exam" style="width:100%;background:linear-gradient(135deg,#6366f1,#8b5cf6);border-radius:.75rem;padding:.85rem;font-size:1.05rem;letter-spacing:.02em;">
...
<div class="main" style="margin-right: 340px;">
    ...
    <form method="post" action="${pageContext.request.contextPath}/candidate/mock-test?action=submit" id="exam-form">
...
function checkIfAnswered(card) {
    ...
    // Check Azure hidden inputs for Speaking
...
// Listen to all inputs to update navigator live
...
// Init navigator for the first active tab on load
setTimeout(() => {
            ...
            try {
                ...
                .then(data => {
                    if(data.success && data.data) {
                        ...
                        updateNavStatus(); // update the grid to show green tick
                    } else {
