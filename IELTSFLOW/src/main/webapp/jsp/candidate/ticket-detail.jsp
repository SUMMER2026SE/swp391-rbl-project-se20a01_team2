                ...
                <main class="main-content" id="appMainContent">
                    <div class="animate-fade-up">
                        ...
                        <c:if test="${not empty param.success}">
                            <div
                                style="background-color: #dafbe1; border: 1px solid #4ac26b; color: #1a7f37; padding: 12px 16px; border-radius: 6px; margin-bottom: 20px; font-size: 14px;">
                                <i class="fa-solid fa-check-circle me-2"></i> ${param.success}</div>
                        </c:if>
                        <c:if test="${not empty error}">
                            <div
                                style="background-color: #ffebe9; border: 1px solid #ff8182; color: #cf222e; padding: 12px 16px; border-radius: 6px; margin-bottom: 20px; font-size: 14px;">
                                <i class="fa-solid fa-circle-exclamation me-2"></i> ${error}</div>
                        </c:if>
                                    ...
                                    <div class="issue-meta-row">
                                        <c:choose>
                                            <c:when test="${ticket.status == 'Closed'}">
                                                <span class="state-badge state-closed"><i
                                                        class="fa-regular fa-circle-check"></i> Closed</span>
                                            </c:when>
                                            <c:otherwise>
                                                <span class="state-badge state-open"><i
                                                        class="fa-regular fa-circle-dot"></i> Open</span>
                                            </c:otherwise>
                                        </c:choose>
                                        <span>
                                            <strong>${ticket.user.fullName}</strong> opened this issue <span
                                                class="time-ago"
                                                data-time="${ticket.createdAt}">${ticket.createdAt}</span> &middot;
                                            ${ticket.replies.size()} comments
                                        </span>
                                    </div>
                                </div>
                                                ...
                                                <div class="timeline-avatar">
                                                    <c:choose>
                                                        <c:when test="${not empty reply.sender.profilePic}">
                                                            <img src="${pageContext.request.contextPath}${reply.sender.profilePic}"
                                                                style="width: 100%; height: 100%; object-fit: cover;">
                                                        </c:when>
                                                        <c:otherwise>${reply.sender.fullName.substring(0, 1)}
                                                        </c:otherwise>
                                                    </c:choose>
                                                </div>
                                                <div class="timeline-content">
                                                    <div class="timeline-header">
                                                        <strong>${reply.sender.fullName}</strong> commented <span
                                                            class="time-ago"
                                                            data-time="${reply.createdAt}">${reply.createdAt}</span>
                                                    ...
                                                    <div class="timeline-body markdown-body raw-markdown"
                                                        style="display:none;">${reply.message}</div>
                                        ...
                                        <c:if test="${ticket.status != 'Closed'}">
                                            <div class="comment-form-container">
                                                <div class="timeline-avatar">
                                                    <c:choose>
                                                        ...
                                                        <c:otherwise>${sessionScope.fullName.substring(0, 1)}
                                                        </c:otherwise>
                                                    </c:choose>
                                                </div>
                                                <div class="comment-form">
                                                    ...
                                                    <form method="POST"
                                                        action="${pageContext.request.contextPath}/candidate/tickets">
                                                        <input type="hidden" name="action" value="reply">
                                                        <input type="hidden" name="ticketId" value="${ticket.ticketId}">
                                                        <textarea name="replyContent" class="comment-textarea"
                                                            placeholder="Leave a comment (Markdown is supported)"></textarea>
                                            ...
                                            <div style="display: flex; justify-content: flex-end; margin-top: 16px;">
                                                <form method="POST"
                                                    ...
                                                    <input type="hidden" name="ticketId" value="${ticket.ticketId}">
                                                    <button type="submit" class="btn btn-danger-outline"><i
                                                            class="fa-regular fa-circle-check"></i> Close issue</button>
                                                </form>
                                            </div>
                                        </c:if>
                                    </div>
