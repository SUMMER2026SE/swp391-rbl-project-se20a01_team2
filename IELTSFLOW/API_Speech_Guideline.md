...
3. Đồng thời, Backend gọi ngầm AI (`AIEvaluationService.evaluateSpeakingAsync`) để phân tích ngữ pháp, từ vựng. Kết quả "Tạo feedback" và "Gợi ý cải thiện kỹ năng" sẽ được lưu JSON vào `AIEvaluations`, còn `SpeakingBand` và `OverallAIFeedback` sẽ tự động được cập nhật lên bảng `TestSubmissions`.
